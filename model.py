...
if os.path.exists("csv_feature.json"):
    with open("csv_feature.json", "r") as input:
        csv_feature_dict = json.load(input)
else:
    ...
    with open("csv_feature.json", "w") as output:
        json.dump(csv_feature_dict, output)
    ...
    def __getitem__(self, i):
        ...
        if self.mode == 'train':
            ...
            return {
                ...
                'name': torch.tensor(int(file_name), dtype=torch.int32),
...
# train = sorted(glob('data/difference/*'))
# test = sorted(glob('data/difference/*'))
...
class RNN_Decoder(nn.Module):
    def __init__(self, max_len, embedding_dim, num_features, class_n, rate):
        ...
        self.final_layer = nn.Linear(1000 + lstm_dim, class_n) # cnn out_dim + lstm out_dim
...
def train_step(batch_item, training):
    ...
    if training is True:
        ...
        with torch.no_grad():
            ...
            #             max = out[w]
            #             max_idx = w
            #     dic.append((int(batch_item['name'][idxx]), label_decoder[max_idx]))
            #     idxx += 1
            # print(f"\n{dic}")
            loss = criterion(output, label)
        score = accuracy_function(label, output)
